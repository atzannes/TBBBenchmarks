...
void parTreeSearch(StealStack *ss) {
  ...
  while (done == 0) {
     ...
     */
#ifdef __LAZY_SCHEDULING
    int amt = (2*chunkSize <= ss->workAvail) ? ss->workAvail/2 : max(ss->workAvail,1);
    if (ss_acquire(ss, amt)) {
      //printf("Called SUCCESS ss_acquire w. amt=%d\n", amt);
      continue;
    }
...
int showStats(double elapsedSecs) {
  int i;
