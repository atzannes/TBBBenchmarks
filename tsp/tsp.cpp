...
volatile list_node* solution = NULL;
...
void solve_tsp_rec(const int (*A)[SIZE], list_node* partial_solution, int position, int partial_weight) 
{
    ...
    tbb::parallel_for (tbb::blocked_range<int>(0,SIZE,GRAINSIZE), tspBody(A,partial_solution,position,partial_weight), partitioner);
}
